import{t as e}from"./shaderStore-D-XQlhUT.js";import"./fogFragment-BjxFNSqh.js";import"./mainUVVaryingDeclaration-B0MsBTDj.js";var t=`pbrFragmentExtraDeclaration`,n=`varying vec3 vPositionW;
#if DEBUGMODE>0
...
{return diffusionProfile<1.;}`;e.IncludesShadersStore[r]||(e.IncludesShadersStore[r]=i);var a=`importanceSampling`,o=`vec3 hemisphereCosSample(vec2 u) {float phi=2.*PI*u.x;float cosTheta2=1.-u.y;float cosTheta=sqrt(cosTheta2);float sinTheta=sqrt(1.-cosTheta2);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}
...
float phi=2.*PI*u.x;float sinTheta=pow(u.y,a/(2.*a+1.));float cosTheta=sqrt(1.-sinTheta*sinTheta);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}`;e.IncludesShadersStore[a]||(e.IncludesShadersStore[a]=o);var s=`pbrHelperFunctions`,c=`#define MINIMUMVARIANCE 0.0005
...
#endif
`;e.IncludesShadersStore[s]||(e.IncludesShadersStore[s]=c);var l=`pbrDirectLightingSetupFunctions`,u=`struct preLightingInfo
...
result.areaLightDiffuse=data.Diffuse;result.LdotV=0.;result.roughness=0.;result.diffuseRoughness=0.;result.surfaceAlbedo=vec3(0.);return result;}
#endif
`;e.IncludesShadersStore[l]||(e.IncludesShadersStore[l]=u);var d=`pbrDirectLightingFalloffFunctions`,f=`float computeDistanceLightFalloff_Standard(vec3 lightOffset,float range)
...
#endif
}`;e.IncludesShadersStore[d]||(e.IncludesShadersStore[d]=f);var p=`hdrFilteringFunctions`,m=`#if NUM_SAMPLES
#if NUM_SAMPLES>0
...
#endif
#endif
`;e.IncludesShadersStore[p]||(e.IncludesShadersStore[p]=m);var h=`pbrDirectLightingFunctions`,g=`#define CLEARCOATREFLECTANCE90 1.0
struct lightingInfo
{vec3 diffuse;
...
#endif
`;e.IncludesShadersStore[_]||(e.IncludesShadersStore[_]=v);var y=`pbrBlockNormalGeometric`,b=`vec3 viewDirectionW=normalize(vEyePosition.xyz-vPositionW);
#ifdef NORMAL
...
#endif
`;e.IncludesShadersStore[y]||(e.IncludesShadersStore[y]=b);var x=`pbrBlockImageProcessing`,S=`#if defined(IMAGEPROCESSINGPOSTPROCESS) || defined(SS_SCATTERING)
...
#endif
`;e.IncludesShadersStore[x]||(e.IncludesShadersStore[x]=S);var C=`pbrBlockPrePass`,w=`#if SCENE_MRT_COUNT>0
...
#endif
#endif
`;e.IncludesShadersStore[C]||(e.IncludesShadersStore[C]=w);var T=`pbrDebug`,E=`#if DEBUGMODE>0
